...
def run_async_in_loop(loop, coro):
    """
    Helper function to run an async coroutine in a synchronous context.
    """
    return loop.run_until_complete(coro)

async def download_progress_hook(d, context, status_msg, task_id, lang, loop):
    """
...
async def upload_progress_hook(file_path, context, status_msg, total_size, lang):
    ...
    chunk_size = 10 * 1024 * 1024  # Increased to 10MB chunks for faster uploads
    last_update = start_time
    update_interval = 1.0  # Update progress bar every 1 second
    ...
    with open(file_path, 'rb') as f:
        while True:
            ...
            # Update progress bar only every `update_interval` seconds
...
async def download_and_upload(url, format_id, user_id, chat_id, context, status_msg, prefix="", lang="en"):
    ...
    # Get the event loop to run async coroutines in the progress hook
    loop = asyncio.get_event_loop()

    # Wrap the async progress hook to make it synchronous for yt_dlp
    sync_progress_hook = lambda d: run_async_in_loop(loop, download_progress_hook(d, context, status_msg, task_id, lang, loop))

    ydl_opts = {
        'outtmpl': f'{DOWNLOAD_DIR}/%(title)s.%(ext)s',
        'progress_hooks': [sync_progress_hook],  # Use the wrapped synchronous hook
        'writesubtitles': True,
        'subtitleslangs': ['en'],
        'noplaylist': True,
        'retries': 10,
    ...
    try:
        ...
        # Truncate error message to avoid "Request Entity Too Large" and remove parse_mode
        error_msg = str(e)[:1000]  # Limit to 1000 characters
