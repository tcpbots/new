...
EXAMPLE_LINKS = {
    ...
    "x.com": "https://x.com/X/status/1234567890123456789",
    "twitter.com": "https://twitter.com/X/status/1234567890123456789",
...
async def upload_progress_hook(file_path, context, status_msg, total_size, lang):
    ...
    while uploaded < total_size:
        ...
        uploaded = min(uploaded + (1024 * 1024), total_size)  # Simulate upload
...
async def download_and_upload(url, format_id, user_id, chat_id, context, status_msg, prefix="", lang="en"):
    ...
    ydl_opts = {
        ...
        'writesubtitles': True,
        'subtitleslangs': ['en'],
        'noplaylist': True,  # Handle playlists separately
...
async def process_file(file_path, thumbnail, subtitles, url, format_id, user_id, chat_id, context, status_msg, prefix, start_time, info):
    ...
    final_path = os.path.join(DOWNLOAD_DIR, f"{info['title'].replace('/', '_')}.{upload_format if format_id != 'audio_only' else 'mp3'}")
