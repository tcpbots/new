...
from database import get_user_data, update_user_data, get_total_users, get_user_activity, get_bot_stats, get_all_user_ids
...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    current_compression = "Yes" if user_data.get("compression", False) else "No"  # New setting
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"Compression: **{current_compression}**", callback_data="toggle_compression"), InlineKeyboardButton(f"Language: **{current_lang}**", callback_data="set_language")]
    ]
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Improved regex to capture more URL formats (e.g., youtu.be, fb.watch, etc.)
    urls = re.findall(r'(https?://(?:www\.)?(?:youtu\.be/|youtube\.com/watch\?v=|fb\.watch/|facebook\.com/(?:watch|share)|x\.com/|twitter\.com/)[^\s]+)', text)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "check_sub":
    ...
    elif query.data == "toggle_compression":  # New setting
        new_compression = not get_user_data(user_id).get("compression", False)
        update_user_data(user_id, {"compression": new_compression})
        await query.edit_message_text(f"**Compression {'enabled' if new_compression else 'disabled'}**", parse_mode='Markdown')
        await log_to_channel(context, f"User {user_id} set compression to {new_compression}")
        ...
        if pending_key in context.user_data:
            ...
            # Immediate download after quality selection
            await status_msg.edit_text("**Starting download...**", parse_mode='Markdown')
            await download_and_upload(url, format_id, user_id, chat_id, context, status_msg, prefix, lang)
