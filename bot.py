...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not valid_urls:
        await update.message.reply_text("**No valid URLs found. Please send a supported video link.**", parse_mode='Markdown')
        return

    await update.message.reply_text(f"**Processing {len(valid_urls)} video(s)...**", parse_mode='Markdown')
    await log_to_channel(context, f"User {user_id} submitted {len(valid_urls)} URL(s)")
...
async def process_single_url(user_id, chat_id, url, context):
    async with asyncio.Semaphore(MAX_CONCURRENT_DOWNLOADS):
        ...
        try:
            ...
            await status_msg.edit_text(f"**Error: Invalid URL or platform not supported - {str(e)}**", parse_mode='Markdown')
...
async def process_video(url, user_id, chat_id, context, status_msg, prefix, lang):
    try:
        ...
        await status_msg.edit_text(f"**{prefix} Error: {str(e)}**", parse_mode='Markdown')
    ...
    if query.data == "check_sub":
        ...
        if pending_key in context.user_data:
            ...
            format_id = quality if quality == "audio_only" else next(
                (f['format_id'] for f in YoutubeDL({'quiet': True}).extract_info(url, download=False)['formats'] if quality[:-1] in str(f.get('height', ''))),
                "best"
            )
            await download_and_upload(url, format_id, user_id, chat_id, context, status_msg, prefix, lang)
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    error = context.error
    try:
        await log_to_channel(context, f"Error occurred: {str(error)}")
